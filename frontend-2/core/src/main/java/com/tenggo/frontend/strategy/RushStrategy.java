...
public class RushStrategy implements AttackStrategy {
    ...
    private RushStrategy.State currentState = RushStrategy.State.CHASING;
        ...
        switch (currentState) {
            case CHASING:
                ...
                if (attackRange.overlaps(player.getHitbox())) {
                    currentState = RushStrategy.State.WINDUP;
            ...
            case WINDUP:
                ...
                if (stateTimer <= 0) {
                    ...
                    enemy.setAttacking(false);
                    currentState = RushStrategy.State.COOLDOWN;
            ...
            case COOLDOWN:
                stateTimer -= delta;
                if (stateTimer <= 0) {
                    currentState = RushStrategy.State.CHASING;
                }
                break;
        }
    }
